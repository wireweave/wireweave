...
import { exportJsonCommand } from './export-json.js'
...
async function makeEnv(): Promise<TestEnv> {
  const workDir = await fs.mkdtemp(path.join(os.tmpdir(), 'wireweave-cli-exjson-'))
...
describe('exportJsonCommand', () => {
  ...
  it('runs locally without fetch (export-json is local-dispatch)', async () => {
  ...
  it('writes output to disk when --output is provided', async () => {
    const file = path.join(env.workDir, 'home.wf')
    const outFile = path.join(env.workDir, 'home.json')
    await fs.writeFile(file, `page {\n  text "hi"\n}\n`, 'utf8')
  ...
  it('returns 1 when source file is missing', async () => {
    ...
    expect(code).toBe(1)
    expect(env.readStderr()).toMatch(/export-json failed/)
  })
})
