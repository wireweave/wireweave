...
이 저장소는 Wireweave의 **공개 표면**(언어 + 도구 + 클라이언트)을 담은 pnpm 모노레포입니다. npm 패키지 8개, 편집기 확장, 문서 사이트로 구성됩니다. 호스티드 AI 에이전트, API 서버, 제품(대시보드·관리자)은 별도의 비공개 저장소에서 관리됩니다.
...
| [`@wireweave/mcp-server`](packages/mcp-server)           | MCP 서버 (SDK 래핑, 로컬 도구는 API 키 불필요)                                                  | npm                    |
...
- `@wireweave/mcp-server` 로컬 도구: API 키 불필요. HTTP 인증과 원격 API 키는 [패키지의 인증 계약](packages/mcp-server/README.md#http-and-credentials)을 따릅니다.
...
- 게시 진입점은 [Publish packages workflow](.github/workflows/publish.yml)이며, 브랜치별 Core 메이저와 npm 채널은 [릴리스 정책](scripts/release-registry.mjs)이 강제합니다.
...
pnpm release:verify       # 로컬 전체 검증, 게시하지 않음
```

승인된 `main` 또는 `develop` 푸시가 발행 워크플로를 실행합니다. 버전 커밋이 푸시된 뒤 재시도하려면 해당 브랜치의 현재 HEAD에서 workflow_dispatch를 실행합니다. 이전 트리거 커밋의 재실행은 원격 HEAD 검사에서 중단됩니다. 워크플로는 게시 전에 발행 의도를 보존하고, 패키지 내용·npm 채널·설치된 MCP 실행을 검증합니다. 누락된 릴리스 태그는 검증된 레지스트리 증거로만 복구하며 기존 태그를 덮어쓰지 않습니다.
